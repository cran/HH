...
mcp2matrix2 <-
function (model, linfct, interaction.average=FALSE, covariate.average=FALSE) 
{
    ...
    for (nm in nhypo) {
        ...
        pos <- factors[nm, ] == 1
        cov.ave <- ((length(covariate.average) == 1 &&
                     (covariate.average=="mean" || covariate.average==TRUE)) ||
                    (length(covariate.average) > 1))
        if (sum(pos) > 1) {
          if (!interaction.average) {
...
##             for (i in which(pos[apply(factors, 2, sum)==1])) { ## rmh []
    ...
    if (!all(classes) && cov.ave) {
      ...
      switch(as.character(length(classes)),
             "0"={},
             "1"= {if (length(covariate.average) != 1)
                     stop("inconsistent covariate.average")
                   c.a <-
                     if (covariate.average=="mean" || covariate.average==TRUE)
                       mean(mf[, ivar, drop = FALSE])
                     else
                       unlist(covariate.average)
  ...
  function (model, linfct, ..., interaction.average=FALSE, covariate.average=FALSE)
{
  tmp <- mcp2matrix2(model, linfct = linfct,
                    interaction.average=interaction.average,
                    covariate.average=covariate.average)
