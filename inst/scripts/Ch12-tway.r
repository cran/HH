...
if.R(s={old.omd <- par(omd=c(.08,1,.05,1))},
     r={})
interaction2wt(time ~ panel + emergenc, data=display,
               par.strip.text=list(cex=if.R(s=1, r=1.1)),
               main.cex=1.6,
...
if.R(r=contrasts(display$panel) <- contr.treatment(3),
     s={})
...
if.R(s={
  ...
  tmp <- multicomp(displayf.aov, "panel", plot=TRUE)
  tmp
},r={
...
if.R(s={
  tmp <- multicomp(displayf.aov, "panel", plot=TRUE, comp="none")
  tmp
...
if.R(s={
  displayf.mmc <- multicomp.mmc(displayf.aov, "panel", plot=TRUE)
  displayf.mmc
},r={
...
if.R(s={
  tmp <- multicomp(displayf.aov, "panel", plot=FALSE)
  print(qtukey(.95, 3, 12)/sqrt(2))##
  print(tmp$crit.point)            ## S-Plus critical value
  print(tmp$crit.point * sqrt(2))  ## SAS: Critical Value of Studentized Range
},r={
  tmp <- glht(displayf.aov, linfct=mcp(
                              panel="Tukey",
                              `interaction_average` = TRUE,
...
if.R(s={
    ms.res <- summary(displayf.aov)["     Residuals","Mean Sq"]
    sqrt(2*ms.res/8) * tmp$crit.point  ## minimum significant difference
},r={
...
t(bwplot(strain ~ Npg | comb, data=rhiz.alfalfa,
         main="alfalfa", layout=c(2,1),
         par.strip.text=list(cex=.9),
         scales=list(cex=.75))
))
print(split = c(1,1,1,2), more = FALSE,  # bottom
t(bwplot(strain ~ Npg | comb, data=rhiz.clover,
         main="clover", layout=c(2,1),
         par.strip.text=list(cex=.9),
         scales=list(cex=.75))
))
...
print(split = c(1,2,1,2), more = TRUE,  # top
t(bwplot(strain ~ nitro | comb, data=rhiz.alfalfa,
         main="alfalfa", layout=c(2,1),
         par.strip.text=list(cex=.9),
         scales=list(cex=.75))
))
print(split = c(1,1,1,2), more = FALSE,  # bottom
t(bwplot(strain ~ nitro | comb, data=rhiz.clover,
         main="clover", layout=c(2,1),
         par.strip.text=list(cex=.9),
         scales=list(cex=.75))
))
...
print(split = c(1,2,1,2), more = TRUE,  # top
t(bwplot(strain ~ weight | comb, data=rhiz.alfalfa,
         main="alfalfa", layout=c(2,1),
         par.strip.text=list(cex=.9),
         scales=list(cex=.75))
))
print(split = c(1,1,1,2), more = FALSE,  # bottom
t(bwplot(strain ~ weight | comb, data=rhiz.clover,
         main="clover", layout=c(2,1),
         par.strip.text=list(cex=.9),
         scales=list(cex=.75))
))
...
if.R(r={
  ...
  alfalfa.mca <- multicomp.reverse(multicomp(rhiz.alfalfa.aov, focus="strain"))
  plot(alfalfa.mca)
  print(alfalfa.mca)
})
...
strain.labels <- paste(dimnames(alf.means$tables$strain)[[if.R(s=2,r=1)]],
...
old.omd <- par(omd=c(0,.85 ,0,1))
alf.mmc <- multicomp.mmc(rhiz.alfalfa.aov, focus="strain", x.offset=.3, ry=c(29.5, 33))
## export.eps(hh("tway/figure/alfalfa.mmc.eps"))
## this plot is hard to read, we need to break the ties and retain the order
plotMatchMMC(alf.mmc$mca)
print(alf.mmc)
...
## construct an orthogonal set of interpretable contrasts
zapsmall(alf.mmc$none$lmat[2:7,]) ## strain rows
##                              c 15 12  1 10  7
alf.comp <- cbind("1,7,10-c"=c(-3, 0, 0, 1, 1, 1),
                  "1,10-7"  =c( 0, 0, 0, 1, 1,-2),
                  "1-10"    =c( 0, 0, 0, 1,-1, 0),
...
alf.lmat <- alf.mmc$none$lmat %*% alf.comp
zapsmall(alf.lmat)[2:7,]
...
model.tables(rhiz.clover.nest.aov, type="means", se=TRUE)
## This gives the anova table we are interested,
## S-Plus multicomp won't work with a nested model.
...
old.omd <- par(omd=c(0,.8,0,1))
## compare simple effects with clover+alfalfa to simple effects with clover
## force these graphs to have the same xlim and ylim as the previous graphs
## simple effects with clover
clover.str.clov.mmc <-
multicomp.mmc(rhiz.clover.aov, focus="strain",
              adjust=list(comb="clover"),
              method="Tukey",
              crit.point=qtukey(1-.05/2, 6, 48)/sqrt(2),
              ry=c(19,38),
              plot=FALSE)
clover.str.clov.mmc <-
  multicomp.label.change(clover.str.clov.mmc, "adj1", "clov")
for (i in 1:2)
  clover.str.clov.mmc[[i]]$method <- "Tukey"
plot(clover.str.clov.mmc)
## mca only, no lmat
## export.eps(h2("mmc/figure/clover.str.clov.mmc.eps"))
## export.eps(h2("mmc/figure/clover.str.clov.mmc-color.eps"))
plotMatchMMC(clover.str.clov.mmc$mca, col.signif=8, lty.signif=1)
## export.eps(h2("mmc/figure/clover.str.clov.mca.eps"))

## construct an orthogonal basis set of contrasts

zapsmall(clover.str.clov.mmc$none$lmat[2:7,]) ## strain rows
##                               5  1  c  7 13  4
clover.comp <- cbind("5-rest"=c( 5,-1,-1,-1,-1,-1),
                   "1-4.7.13"=c( 0, 3, 0,-1,-1,-1),
                 "c-1.4.7.13"=c( 0,-1, 4,-1,-1,-1),
...
clover.lmat <- clover.str.clov.mmc$none$lmat %*% clover.comp
zapsmall(clover.lmat)[2:7,]


clover.str.clov.lmat.mmc <-
multicomp.mmc(rhiz.clover.aov, focus="strain",
              adjust=list(comb="clover"),
              lmat=clover.lmat,
              method="Tukey",
              ...
              plot=FALSE)
clover.str.clov.lmat.mmc <-
  multicomp.label.change(clover.str.clov.lmat.mmc, "adj1", "clov")
for (i in 1:3)
  clover.str.clov.lmat.mmc[[i]]$method <- "Tukey"
plot(clover.str.clov.lmat.mmc)
## lmat only, no mca
## export.eps(h2("mmc/figure/clover.str.clov.lmat.mmc.eps"))
plotMatchMMC(clover.str.clov.lmat.mmc$lmat, col.signif="blue")
## tiebreaker not needed, but here it is anyway


## simple effects with clover+alfalfa
clover.str.clovalf.mmc <-
multicomp.mmc(rhiz.clover.aov, focus="strain",
              adjust=list(comb="clover+alfalfa"),
              method="Tukey",
              crit.point=qtukey(1-.05/2, 6, 48)/sqrt(2),
              ry=c(19,38),
              plot=FALSE)
clover.str.clovalf.mmc <-
  multicomp.label.change(clover.str.clovalf.mmc, "adj1", "clovalf")
for (i in 1:2)
  clover.str.clovalf.mmc[[i]]$method <- "Tukey"
plot(clover.str.clovalf.mmc)
## mca only of clover+alfalfa
## export.eps(h2("mmc/figure/clover.str.clovalf.mmc.eps"))
plotMatchMMC(clover.str.clovalf.mmc$mca, col.signif="blue")
## export.eps(h2("mmc/figure/clover.str.clovalf.mca.eps"))
par(old.omd)
## identification of significant contrasts not needed because nothing is significant
})
...
old.par <- if.R(r=par(mfrow=c(2,1)),
                s=par(mfrow=c(1,2)))
...
## if.R(s=
##      attr(plasma$time, "position") <- c(1,3,5,7,9)+.5
##      ,r={})
...
if.R(s=
     print(position=c(0,.4, 1,1),
           t(bwplot(station ~ devices | method, data=workstation,
                    xlab=list(cex=1.4),
                    ylab=list("station %in% method", cex=1.4),
